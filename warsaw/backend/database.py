...
class DB:
    ...
    def getmatches(self, uid):
        ...
        try:
            ...
            qry = f'''
            select
            m.match matched_username, 
            p.userdata:"personalInformation" personalInfo
            from
            matches m
            inner join profile p
            on m.match = p.username
            where m.user = '{uid}'
            and m.match in (select user from matches where match = '{uid}')
            '''
            # print(qry)
            ret = cur.execute(qry)
            for record in ret:
                foo.__setitem__(record[0], json.loads(record[1]))
            return foo
        except Exception as e:
            # print(e)
            return  jsonify({'Error': str(e)})
    ...
    def getuserscores(self, uid):
        ...
        try:
            ...
            for record in ret:
                bar = {}
                bar.__setitem__('SCORE', record[1])
                bar.__setitem__('GAMEPROFILE', json.loads(record[2]))
                foo.__setitem__(record[0], bar) 
        except Exception as e:
            print(e)
            return  jsonify({'Error': str(e)})
        finally:
